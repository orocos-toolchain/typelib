...
BOOST_AUTO_TEST_CASE( test_vector_assumptions )
{
    ...
    BOOST_REQUIRE_EQUAL(10 * sizeof(int) / sizeof(Container), reinterpret_cast< vector<Container>* >(&values)->size());
}
...
BOOST_AUTO_TEST_CASE( test_vector )
{
    ...
    Container* container = Container::createContainer("std/vector", *registry.get("B"));
...
{ 
    ...
    Container* container = Container::createContainer("std/vector", *registry.get("int32_t"));
    BOOST_REQUIRE(container);
